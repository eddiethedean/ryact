"""
Extract Jest describe/it/test titles from React scheduler __tests__ JavaScript files.

Used for Milestone 4 inventory + drift detection. Best-effort string/comment skipping;
extend skip_non_code if upstream adds edge-case syntax.
"""
...
def stable_case_id(upstream_path: str, describe_path: tuple[str, ...], it_title: str) -> str:
    ...
    return f"scheduler.{stem}.{dslug}.{tslug}"
...
def extract_all(upstream_root: Path) -> list[ExtractedCase]:
    ...
    for rel in SCHEDULER_TEST_FILES:
