#!/usr/bin/env python3
"""
Fail if facebook/react React DOM __tests__ contain Jest cases not listed in
tests_upstream/react_dom/upstream_inventory.json.

Usage:
  python scripts/check_react_dom_upstream_inventory.py /path/to/react
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python scripts/check_react_dom_upstream_inventory.py /path/to/facebook/react",
            file=sys.stderr,
        )
        return 2
    ...
    if missing:
        print("Upstream React DOM tests not covered by upstream_inventory.json:", file=sys.stderr)
        for k in missing[:50]:
            print(f"  {k}", file=sys.stderr)
        if len(missing) > 50:
            print(f"  ... and {len(missing) - 50} more", file=sys.stderr)
        print(
            f"\nRun: python scripts/update_react_dom_upstream_inventory.py {upstream_root}",
            file=sys.stderr,
        )
        return 1
    ...
    print("OK: react_dom upstream_inventory matches extracted Jest cases.")
    return 0
