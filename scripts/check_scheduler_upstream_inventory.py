#!/usr/bin/env python3
"""
Fail if facebook/react scheduler __tests__ contain Jest cases not listed in
tests_upstream/scheduler/upstream_inventory.json.

Usage:
  python scripts/check_scheduler_upstream_inventory.py /path/to/react
...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "Usage: python scripts/check_scheduler_upstream_inventory.py /path/to/react",
            file=sys.stderr,
        )
        return 2
    ...
    from scripts.scheduler_jest_extract import canonical_case_key, extract_all
    ...
    if missing:
        print("Upstream scheduler tests not covered by upstream_inventory.json:", file=sys.stderr)
        for k in missing[:50]:
            print(f"  {k}", file=sys.stderr)
        if len(missing) > 50:
            print(f"  ... and {len(missing) - 50} more", file=sys.stderr)
        print(
            f"\nRun: python scripts/update_scheduler_upstream_inventory.py {upstream_root}",
            file=sys.stderr,
        )
        return 1
    ...
    print("OK: scheduler upstream_inventory matches extracted Jest cases.")
    return 0
