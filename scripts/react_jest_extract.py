"""
Extract Jest describe/it/test titles from React core __tests__ JavaScript files.

Modeled after ``scripts/scheduler_jest_extract.py`` but generalized to scan
...
def stable_case_id(upstream_path: str, describe_path: tuple[str, ...], it_title: str) -> str:
    ...
    return f"react.{stem}.{dslug}.{tslug}"
...
def extract_all(upstream_root: Path) -> list[ExtractedCase]:
    ...
    for rel in iter_react_test_files(upstream_root):
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python scripts/react_jest_extract.py /path/to/facebook/react")
        return 2
