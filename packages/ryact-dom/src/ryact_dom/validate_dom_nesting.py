...
_IMPLIED_END_TAGS: frozenset[str] = frozenset({"dd", "dt", "li", "option", "optgroup", "p", "rp", "rt"})
...
def _is_tag_valid_with_parent(tag: str, parent_tag: str | None, implicit_root_scope: bool) -> bool:
    ...
    if parent_tag == "html" and not implicit_root_scope:
        return tag in {"head", "body", "frameset"}
    if parent_tag == "frameset":
        return tag == "frame"
    if parent_tag == "#document" and not implicit_root_scope:
        return tag == "html"
    ...
    if tag == "body":
        return (implicit_root_scope and parent_tag in ("#document", "html")) or parent_tag is None
...
def validate_dom_nesting_host_child_dev(
    *,
    ...
    invalid_parent_info = (
        None if _is_tag_valid_with_parent(child_tag_l, parent_tag, ancestor_info.implicit_root_scope) else parent_info
    )
    invalid_ancestor = (
        None if invalid_parent_info is not None else _find_invalid_ancestor_for_tag(child_tag_l, ancestor_info)
    )
    ...
    if invalid_parent_info is not None:
        ...
        if ancestor_tag == "table" and child_tag_l == "tr":
            extra = " Add a <tbody>, <thead> or <tfoot> to your code to match the DOM tree generated by the browser."
        msg = (
