"""
``Scheduler.js`` harness for the ``setTimeout(0)`` fallback host (``SchedulerNoDOM`` tests).

Uses a timer facade with ``set_timeout(..., 0)`` + ``run_all_pending`` (``jest.runAllTimers``).
"""
...
class SetTimeoutSchedulerHarness:
    """``unstable_*`` surface with ``setTimeout(0)``-scheduled flushes."""

    def __init__(
        self,
        set_timeout: Callable[[Callable[[], None], int], int],
