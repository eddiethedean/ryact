...
Every inventory row for [`packages/scheduler/src/__tests__`](https://github.com/facebook/react/tree/main/packages/scheduler/src/__tests__) is **`implemented`** or **`non_goal`** once **M13** is **(done)**. **M10** **(done)** — inventory regen + drift check + triage docs; **M11** **(done)** — heap-triaged **`Scheduler-test.js`** **`pending`** cleared at current **`main`** (playbook for future regen); **M12** ports browser-triaged **`pending`**; **M13** closes **`pending`** to zero. Milestone **9** supplies shared flags/docs/dedupe so closure does not re-open harness drift.
