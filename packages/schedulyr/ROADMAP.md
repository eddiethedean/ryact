...
**Model note:** the default **[`Scheduler`](src/schedulyr/scheduler.py)** uses **timer** and **task** min-heaps with per-priority **expiration** (Milestone **14** first slice — aligns with React **`Scheduler.js`** core queues; see **`SCHEDULER_ENTRYPOINTS.md`** parity matrix). Host scheduling remains separate: **`MessageChannel`**, **`setImmediate`**, **`setTimeout(0)`**, **`scheduler.postTask`**. Milestone **5** — **[`BrowserSchedulerHarness`](src/schedulyr/browser_scheduler.py)** (`MessageChannel`); **6** — **[`UnstableMockScheduler`](src/schedulyr/mock_scheduler.py)** (mock); **7** — **[`PostTaskSchedulerHarness`](src/schedulyr/post_task_scheduler.py)**, **[`SetImmediateSchedulerHarness`](src/schedulyr/set_immediate_scheduler.py)**, **[`SetTimeoutSchedulerHarness`](src/schedulyr/set_timeout_scheduler.py)**; **8** — mock **profiling** buffer + **`unstable_profiling`** (**`SchedulerProfiling-test.js`**); **9** — **[`scheduler_browser_flags.py`](src/schedulyr/scheduler_browser_flags.py)** (`SchedulerBrowserFlags`), shared **[`_browser_style_work_loop.py`](src/schedulyr/_browser_style_work_loop.py)** for browser / set-immediate / set-timeout harnesses, **[`SCHEDULER_ENTRYPOINTS.md`](SCHEDULER_ENTRYPOINTS.md)** (Python vs upstream map + **`ryact`** embedder note); **10** — inventory regen verified against **`main`**, triage table in **`SCHEDULER_ENTRYPOINTS.md`**; **11** — heap-first **`Scheduler-test.js`** closure verified (**0** heap-triaged **`pending`** at current **`main`**; playbook under M11); **12** — browser **`Scheduler-test.js`** slice verified (**0** **`pending`**; **`SchedulerBrowser`** **`implemented`**; playbook under M12); **13** — full **`packages/scheduler/src/__tests__`** inventory closure at **`main`** (**0** **`pending`**; **`test_upstream_inventory_schema.test_scheduler_inventory_has_no_pending`**). **“100% parity” B** (inventory) is **(done)** at the pinned **`upstream_ref`**. **Milestone 14** **(done)** — first **Parity C** slice on default **`Scheduler`** (see § **Milestone 14**). **Milestone 15** **(first slice done)** — **`run_until_idle(max_tasks=...)`** (see § **Milestone 15**). **Milestones 16–17** are **(not started)** (**`ryact`** lane integration, optional cross-runtime). **Feature flags** for new parity tests should import **`SchedulerBrowserFlags`** from **`schedulyr.scheduler_browser_flags`** (or **`schedulyr`**) so M10–M12 do not re-split flag defaults.

---

## Baseline today (implemented sketch)

- **Priority constants** — `IMMEDIATE_PRIORITY` … `IDLE_PRIORITY` (numeric ordering; ties at equal **expiration** break by monotonic task **id**; priority affects **expiration** via React timeout table — see [src/schedulyr/scheduler.py](src/schedulyr/scheduler.py)).
- **`Scheduler`** — injectable **`now`** (defaults to **`time.monotonic`**); **timer** heap (delayed work keyed by **`start_time`**) and **task** heap (ready work keyed by **`expiration_time`**, then **`id`**) matching React **`Scheduler.js`** promotion and ordering for the public API (Milestone **14**).
- **`schedule_callback`** — returns task **`id`**; callbacks are **`Callable[[], Any]`** (return **`None`** or a **0-arg** continuation callable); **`delay_ms < 0`** is clamped to **`0`**; delayed work uses **`start_time = now() + delay_ms/1000`** then **`expiration_time = start_time + timeout(priority)`**.
- **`cancel_callback(task_id)`** — lazy cancellation (skipped when the task is popped).
- **Continuations** — if a callback **returns** a callable, it is queued as new work (same **priority**, **`start_time` / `expiration` from `now()`** after the callback).
- **`run_until_idle(time_slice_ms=None, *, max_tasks=None)`** — advances timers, drains the task heap until idle or the next timer is in the future; **`time_slice_ms`** sets a deadline checked **before each task and after each callback**; optional **`max_tasks`** caps callback invocations per call (Milestone **15**). **`time_slice_ms=0`** means deadline equals **`now()`** at entry, so **no** tasks run until **`now`** moves; **`max_tasks=0`** yields before any callback.
- **`default_scheduler`** — module-level instance (use sparingly in tests; prefer explicit **`Scheduler()`**).

Still **not** in standalone **[`Scheduler`](src/schedulyr/scheduler.py)** (later slices / harnesses): **frame-aligned** **`shouldYieldToHost`**, **priority starvation** relief beyond embedder chunking (**`max_tasks`**), **rAF** / **IdleCallback** emulation. **`BrowserSchedulerHarness`** (Milestone 5) implements **MessageChannel**-style macrotasks + **`shouldYield`** / **`requestPaint`** for the **`SchedulerBrowser`** slice only. **`UnstableMockScheduler`** (Milestone 6) implements React’s **`scheduler/unstable_mock`** fork (**virtual time**, explicit flushes, **`log`** / **`waitFor`**-style helpers); Milestone **8** adds opt-in **`enable_profiling`** + **`scheduler_profiling_buffer`**. Milestone **7** adds **`PostTaskMockRuntime`** / **`PostTaskSchedulerHarness`** (**`Post Task`** / **`Task Fired`** / **`Yield`** logs), **`SetImmediateMockRuntime`** / **`SetImmediateSchedulerHarness`** (**`Set Immediate`** / **`setImmediate Callback`**), and **`SetTimeoutSchedulerHarness`** (**`FakeTimers.run_all_pending`**). Milestone **9** centralizes **`SchedulerBrowserFlags`** and documents which module owns which upstream surface — see **[`SCHEDULER_ENTRYPOINTS.md`](SCHEDULER_ENTRYPOINTS.md)** and the table below.

**Entrypoint vs `ryact` (canonical split):**

| Python | Upstream | `ryact` uses it? |
|--------|----------|------------------|
| [`scheduler.py`](src/schedulyr/scheduler.py) `Scheduler` | `Scheduler.js` timer/task + expiration (M14); manifest rows: first five heap + **`scheduler.productionWorkLoop`** | **Yes** — [`reconciler.py`](../ryact/src/ryact/reconciler.py) default cooperative scheduler only |
...
**Errors:** if a callback raises, the exception **propagates** out of **`run_until_idle`**; the failed task was already popped and is not re-run; remaining queued work is left for a **subsequent** **`run_until_idle()`** (no “swallow and continue” unless a future manifest test requires it).
...
- **`ryact.reconciler.Root`** accepts an optional default **`schedulyr.Scheduler`** only (not browser/fork harnesses; see **[`SCHEDULER_ENTRYPOINTS.md`](SCHEDULER_ENTRYPOINTS.md)**); **`schedule_update_on_root`** maps **`Lane`** → scheduler priority and schedules a coalesced flush that runs **`perform_work`** (see [`reconciler.py`](../ryact/src/ryact/reconciler.py): **`bind_commit`**, **`lane_to_scheduler_priority`**).
...
The standalone **[`Scheduler`](src/schedulyr/scheduler.py)** class is unchanged for consumers that only need the cooperative **`run_until_idle`** API (no browser host).
...
## Milestone 14 — Production **`Scheduler.js`** work-loop parity **(done — first slice)**

**Why this milestone exists:** **Parity B** (**M13**) closes the **Jest inventory**. **Parity C** still needed **`schedulyr.Scheduler`** aligned with React **`Scheduler.js`** **timer** vs **task** queues and **expiration** ordering.

**Delivered (this slice):**

- **[`scheduler.py`](src/schedulyr/scheduler.py)** — **`Scheduler`** refactored **in place**: **`_timer_heap`** / **`_task_heap`**, **`_advance_timers`**, **`_expiration_offset_seconds`** (same timeout table as **[`mock_scheduler.py`](src/schedulyr/mock_scheduler.py)**, seconds to match **`now()`**). Public API unchanged for **`ryact`**.
- **Parity matrix** — **[`SCHEDULER_ENTRYPOINTS.md`](SCHEDULER_ENTRYPOINTS.md)** § *Milestone 14 — `Scheduler.js` work-loop parity matrix*.
- **Tests + manifest** — **[`test_scheduler_production_work_loop.py`](../../tests_upstream/scheduler/test_scheduler_production_work_loop.py)** + **`MANIFEST`** **`scheduler.productionWorkLoop`**; the five legacy heap rows unchanged and still green.

**Deferred to M15+ / follow-up:** frame-aligned **`shouldYieldToHost`** inside default **`Scheduler`** (Python keeps **`time_slice_ms`** as the embedder yield knob); **`needsPaint`**; starvation policy beyond expiration ordering—only when tests require them.

**Exit criteria (M14):** met for this scope — matrix checked in; **Parity B** green; no regression on heap / harness suites; new semantics encoded by **`scheduler.productionWorkLoop`**.

**Non-goals (M14):** duplicating **browser / mock / fork** harnesses inside **`scheduler.py`**—those remain separate modules (**M5–M8**).

---

## Milestone 15 — Fairness, starvation, and idle / paint surfaces **(done — first slice)**

**Why this milestone exists:** the **Baseline** section still defers full **starvation / fairness** guarantees and **`requestAnimationFrame`** / **IdleCallback**-style scheduling until **manifest-driven** tests exist. This slice adds an **embedder-controlled** cooperative drain cap on the default **`Scheduler`**; further M15 work follows the same test-first playbook when upstream or **`ryact`** adds drivers.

**Delivered (first slice):**

- **`run_until_idle(..., max_tasks=...)`** on **[`Scheduler`](src/schedulyr/scheduler.py)** — optional keyword-only cap on **callback invocations** per drain (with **`max_tasks=0`** yielding before any **`cb()`**); composes with **`time_slice_ms`**. No change to call sites that omit **`max_tasks`**.
- **Tests + manifest** — **[`test_scheduler_fairness.py`](../../tests_upstream/scheduler/test_scheduler_fairness.py)**, **`MANIFEST`** **`scheduler.fairness.cooperativeDrain`**.
- **Contract** — **[`SCHEDULER_FAIRNESS_CONTRACT.md`](../../tests_upstream/scheduler/SCHEDULER_FAIRNESS_CONTRACT.md)**.

**When this fires again:** upstream **`pending`** rows (fairness / idle / paint), new **`ryact-dom`** integration tests, or additional synthetic **`scheduler.fairness.*`** / harness extensions — follow the **Spec / Code / Docs** tracks below; keep **Parity B** green.

| Track | Deliverables |
|-------|----------------|
| **Spec** | Link each behavior to a **`MANIFEST`** row + **`upstream_inventory.json`** row (or new upstream case after regen), or synthetic **`scheduler.*`** + contract doc. |
...
**Exit criteria (M15):** every new behavior is **test-backed**; **Parity A/B** unchanged or extended via inventory + manifest (this slice: synthetic only).
...
| **Benchmarks** | **`pyperf`** / **`pytest-benchmark`**-style harness for **`Scheduler`** work-loop cost (regression detection only; no competitive claims). |
...
Every row you already track in **`tests_upstream/MANIFEST.json`** for **`schedulyr`** / scheduler integration is **`implemented`** and passing (**`Scheduler`** (including **`scheduler.productionWorkLoop`**, **`scheduler.fairness.cooperativeDrain`**), **`SchedulerBrowser`**, **`scheduler.mock.*`**, **`scheduler.fork.*`**, plus **`react_dom.createRoot.schedulerIntegration`** where listed).
...
**Note:** **Milestone 5** adds **`MockBrowserRuntime`** (**`Post Message` / `Message Event`**) for **`SchedulerBrowser`** parity. **Milestone 6** adds **`UnstableMockScheduler`** for **`unstable_mock`**. **Milestone 7** adds fork hosts (**`PostTask*`**, **`SetImmediate*`**, **`SetTimeout*`** harnesses). **Milestone 8** adds **`enable_profiling`** on the mock scheduler for **`SchedulerProfiling-test.js`**. **Milestones 10–13** **(done)** — full scheduler **`__tests__`** inventory at **`main`** (**M13** = zero **`pending`**). **Milestone 14** **(done)** — default **`Scheduler`** matches React **`Scheduler.js`** core timer/task + expiration ordering (no browser hosts in **`scheduler.py`**). **Milestone 15** **(first slice done)** — **`run_until_idle(max_tasks=...)`** on default **`Scheduler`**; further fairness / idle / paint when tests land. **Milestones 16–17** chart remaining **parity C**—**`ryact`** integration, optional cross-runtime checks—see definitions above.
